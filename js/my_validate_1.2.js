...
(function($) { // 开启沙箱模式
	$(function() {
		...
		$.fn.my_validate = function(obj) {
			...
			var chineseLenth = obj["chineseLenth"] != undefined ? obj["chineseLenth"] : false;
			...
			function eachVali(val, i) {
				for (var j = 0; j < myJSON[i].rules.length; j++) {
					...
					// 第二种,长度校验
						 ...
						 if(chineseLenth){
							 if(strlen(val) > parseInt(myJSON[i].rules[j].longest) || strlen(val) < parseInt(myJSON[i].rules[j].shortest)){
								 errorTips(myJSON[i].rules[j].tips);
								 break;
							 }else{
								 removeHtml();
